  ...
  return (
    <div className="">
      ...
      <div className="mt-2 mx-auto w-full">
        ...
        <div className="grid grid-cols-7 border-blue-400 border items-center justify-center text-center">
          {weekdays.map((day, index) => (
            <Cell
              key={index}
              className="h-12 flex items-start justify-end cursor-pointer"
            >
              <p className="m-auto text-xl text-pink-500">{day}</p>
            </Cell>
          ))}
        </div>
        ...
        <div className="mt-10 grid grid-cols-7 items-center justify-center text-center">
          {Array.from({ length: prefixDays }).map((_, index) => {
            return <Cell className="bg-black-400" key={index} />;
          })}
          ...
          {Array.from({ length: numDays }).map((_, index) => {
            ...
            return (
              <Cell
                key={date}
                className="text-md bg-black-600 hover:bg-green-50 hover:text-4xl text-pink-500
                flex items-start justify-end cursor-pointer transition-all duration-700                
                "
              >
                <p>{date}</p>
              </Cell>
          ...
          {Array.from({ length: suffixDays }).map((_, index) => {
            return <Cell key={index} className="bg-black-400" />;
          })}
        </div>
      </div>
    </div>
